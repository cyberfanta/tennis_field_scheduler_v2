class UiTextEn {
  String get title {
    return "Tennis Field Scheduler - v.2";
  }

  String get tennis {
    return "Tennis";
  }

  String get court {
    return "court";
  }

  String get begin {
    return "Begin";
  }

  String get reserve {
    return "Reserve";
  }

  String get reservations {
    return "Reserves";
  }

  String get favorites {
    return "Favorites";
  }

  String get goLogin {
    return "Log In";
  }

  String get goSignUp {
    return "Sign Up";
  }

  String get hintName {
    return "Name and last name";
  }

  String get nameError {
    return "Check your name";
  }

  String get email {
    return "Email";
  }

  String get hintEmail {
    return "Write your email";
  }

  String get emailError {
    return "Check your email";
  }

  String get pass {
    return "Password";
  }

  String get confirmPass {
    return "Confirm password";
  }

  String get hintPass {
    return "Write your password";
  }

  String get passError1 {
    return "Your password must have one lowercase letter, one uppercase letter, one number, and one character between \$@!%*?&_.";
  }

  String get passError2 {
    return "Your password is too short";
  }

  String get confirmPassError {
    return "Password do not match";
  }

  String get hintPhone {
    return "Phone";
  }

  String get phoneError {
    return "Check your phone";
  }

  String get rememberPass {
    return "Remember password";
  }

  String get forgotYourPass {
    return "Forgot your password?";
  }

  String get stillNotAccount {
    return "Need an account?";
  }

  String get youHaveAccount {
    return "I have an account";
  }

  String get warning {
    return "Warning!!!";
  }

  String get info {
    return "Information!!!";
  }

  String get closeAppContext {
    return "You are about to close the app.\nAre you sure?";
  }

  String get yes {
    return "Yes";
  }

  String get no {
    return "No";
  }

  String get notImplementedContext {
    return "This functionality still not implemented";
  }

  String get ok {
    return "Ok";
  }

  String get nameEmpty {
    return "Can not login - Name is empty";
  }

  String get emailEmpty {
    return "Can not login - Email is empty";
  }

  String get phoneEmpty {
    return "Can not login - Phone is empty";
  }

  String get passEmpty {
    return "Can not login - Password is empty";
  }

  String get noData {
    return "Can not login - No login data available";
  }

  String get wrongCredential {
    return "Can not login - Credential do not match";
  }

  String get fields {
    return "Fields";
  }

  String get hello {
    return "Hello";
  }

  String get scheduledReserves {
    return "Scheduled reserves";
  }

  String get scheduleReserve {
    return "Schedule a reserve";
  }

  String get myReserves {
    return "My reserves";
  }

  String get myFavorites {
    return "My favorites";
  }

  String get fieldA {
    return "Field type A";
  }

  String get fieldB {
    return "Field type B";
  }

  String get fieldC {
    return "Field type C";
  }

  String get available {
    return "Available";
  }

  String get notAvailable {
    return "Not available";
  }

  String get perHour {
    return "Per hour";
  }

  String get addTeacher {
    return "Add teacher";
  }

  String get dateTime {
    return "Set date and time";
  }

  String get date {
    return "Date";
  }

  String get initHour {
    return "Begin hour";
  }

  String get endHour {
    return "End hour";
  }

  String get addComment {
    return "Add comment";
  }

  String get reservedBy {
    return "Reserved by";
  }

  String get hours {
    return "hours";
  }

  String get noSchedules {
    return "There are not schedules yet";
  }

  String get noFavorites {
    return "There are not favorites yet";
  }

  String get wrongDates {
    return "Starting date must be before the ending date";
  }

  String get addedFavorite {
    return "Item added to favorites";
  }
}
