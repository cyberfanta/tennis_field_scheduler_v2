...
class ReservationsViewUseCases {
  ...
  Future<void> Function() goReserve(String tag, BuildContext context) =>
      () async {
  ...
  Future<void> Function() deleteSchedule(String tag, int index) => () async {
        stamp(tag, "Gesture Used: \"deleteSchedule\"",
            decoratorChar: " * ", extraLine: true);

        final List<ScheduledField> newList =
            List<ScheduledField>.from(scheduleList.value);
